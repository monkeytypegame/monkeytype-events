...
interface TextareaInputProps {
  onAttachListeners: (element: HTMLTextAreaElement) => () => void;
  onClear: () => void;
}
...
export function TextareaInput({
  ...
  useEffect(() => {
    ...
    const cleanup = onAttachListeners(textarea);
  ...
  return (
    <div className="w-full space-y-4">
      {/* Header with logo, title, and clear button */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          ...
          >
            <svg
              className="h-12 w-12 hover:drop-shadow-lg transition-all duration-300"
        ...
        <button
          onClick={handleClear}
          className="bg-error hover:bg-error/80 text-white font-medium px-4 py-2 rounded-lg transition-colors duration-200 shadow-md hover:shadow-lg whitespace-nowrap"
        >
          Clear Events
        </button>
      </div>
      ...
      <div className="w-full">
        <textarea
          ref={textareaRef}
          className="w-full h-32 p-4 bg-sub-alt border border-sub/20 rounded-lg text-text placeholder-sub resize-none focus:outline-none focus:ring-2 focus:ring-main/50 focus:border-main transition-colors duration-200"
          placeholder="Start typing to capture keyboard events..."
          autoFocus
        />
      </div>
    </div>
