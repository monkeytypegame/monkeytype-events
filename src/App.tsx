...
function App() {
  ...
  return (
    <div className="min-h-screen bg-bg flex items-center justify-center p-4">
      <div className="bg-sub-alt rounded-2xl shadow-xl p-8 max-w-md w-full text-center space-y-6 border border-sub/20">
        <div className="flex justify-center items-center space-x-8 mb-8">
          ...
          >
            <img
              src={viteLogo}
              className="h-16 w-16 hover:drop-shadow-lg transition-all duration-300 hover:brightness-110"
              alt="Vite logo"
            />
          </a>
          <a
        ...
        <h1 className="text-4xl font-bold text-text mb-6">Monkeytype Events</h1>

        <div className="bg-bg rounded-xl p-6 space-y-4 border border-sub/10">
          <button
            onClick={() => setCount((count) => count + 1)}
            className="bg-main hover:bg-main/80 text-bg font-semibold py-3 px-6 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 active:scale-95 border-2 border-main hover:border-main/80"
          >
            count is {count}
          </button>
          <p className="text-sub text-sm">
            Edit{" "}
            <code className="bg-sub-alt px-2 py-1 rounded text-xs font-mono text-text border border-sub/20">
              src/App.tsx
            </code>{" "}
        ...
        <p className="text-sub text-sm mt-6">
          Click on the logos to learn more about the tools
        </p>
      </div>
    </div>
  );
